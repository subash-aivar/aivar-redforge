...
- **Authorization** — two structurally separate planes: platform `SUPER_ADMIN` (bootstrap via `REDFORGE_PLATFORM_BOOTSTRAP_*`, step-by-step provisioning in [`docs/RUNBOOK_PLATFORM_SUPER_ADMIN_BOOTSTRAP.md`](docs/RUNBOOK_PLATFORM_SUPER_ADMIN_BOOTSTRAP.md)) and per-organization `MembershipRole` (OWNER/ADMIN/SECURITY_MANAGER/ANALYST/MEMBER/VIEWER). An organization Owner/Admin role never grants platform authority — that's enforced structurally, not just at the UI layer, with a "last super-admin can't be removed" invariant.
