...
ROOT = Path(__file__).resolve().parents[2] / "src" / "siem_search"
...
SIEM_CONTEXT_IMPORT = re.compile(
    ...
    r"siem_correlation|siem_investigation|siem_alerting|siem_analytics)\."
)
...
def test_owns_no_aggregate() -> None:
    """M37 §10 — siem_search is thin, mostly-infrastructure, with
    minimal domain logic of its own."""
...
def test_domain_layer_does_not_import_sibling_siem_contexts() -> None:
    """Tier selection is deferred to the application layer via a port
    (M42 Phase 10) — the domain layer must not reach into siem_storage
    or any other sibling context directly."""
    for path in (ROOT / "domain").rglob("*.py"):
        text = path.read_text()
...
def test_no_out_of_scope_engine_vocabulary() -> None:
    """M44E's own explicit scope guard: this milestone implements
    search only — no analytics, statistics, dashboards, trend
    calculations, or risk scoring may appear anywhere in this bounded
    context's application-layer source."""
    banned = re.compile(
        r"\b(Analytics|Statistic(s)?|Dashboard|TrendCalculat(e|ion)|"
        r"RiskScor(e|ing)|Aggregat(e|ion)Metric)\b"
    )
    for path in (ROOT / "application").rglob("*.py"):
        text = path.read_text()
...
def test_search_engine_is_read_only_no_writer_port_exists() -> None:
    """M44E's special review: search remains read-only — unlike every
    other SIEM engine milestone (M43C-M44D), this context defines no
    outbound writer/receiver port at all, because there is nothing for
    it to persist or hand off. A future `IWriter`/`IReceiver`-shaped
    port appearing here would itself be the scope violation."""
    port_files = [p.name for p in (ROOT / "application" / "ports").glob("*.py")]
    banned_names = {"i_search_writer.py", "i_search_receiver.py", "i_event_receiver.py"}
    assert not (banned_names & set(port_files)), port_files


def test_search_application_service_holds_no_growing_state() -> None:
    """Same discipline as M44B-M44D: the service must carry only its
    injected collaborators between calls, never a dict/list attribute
    that would grow unboundedly."""
    text = (ROOT / "application" / "services" / "search_application_service.py").read_text()
