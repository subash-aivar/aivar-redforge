...
from .conftest import FakeCorrelationEvaluator


def test_register_then_resolve_returns_same_evaluator() -> None:
    registry = InMemoryCorrelationEvaluatorRegistry()
    evaluator = FakeCorrelationEvaluator("rule-1", SchemaVersion(1, 0))
...
def test_resolve_matches_compatible_minor_version() -> None:
    registry = InMemoryCorrelationEvaluatorRegistry()
    evaluator = FakeCorrelationEvaluator("rule-1", SchemaVersion(1, 3))
    registry.register(evaluator)
...
def test_is_registered() -> None:
    registry = InMemoryCorrelationEvaluatorRegistry()
    registry.register(FakeCorrelationEvaluator("rule-1", SchemaVersion(1, 0)))
...
def test_different_rules_do_not_conflict() -> None:
    registry = InMemoryCorrelationEvaluatorRegistry()
    registry.register(FakeCorrelationEvaluator("rule-1", SchemaVersion(1, 0)))
    registry.register(FakeCorrelationEvaluator("rule-2", SchemaVersion(1, 0)))
...
def test_resolve_ambiguous_when_two_compatible_versions_registered() -> None:
    registry = InMemoryCorrelationEvaluatorRegistry()
    registry.register(FakeCorrelationEvaluator("rule-1", SchemaVersion(1, 0)))
    registry.register(FakeCorrelationEvaluator("rule-1", SchemaVersion(1, 5)))
