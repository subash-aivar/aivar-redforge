"""FastAPI fixtures for threat_report_intel API integration tests —
mirrors `tests/infrastructure_intel/api/conftest.py`'s shape exactly: a
real ASGI app with the threat_report_intel router mounted,
`TenantContext`/`PlatformContext` overridden via FastAPI's
`dependency_overrides` (never a spoofable request header), and a real
Postgres-backed `ThreatReportIntelContainer`."""
...
from threat_report_intel.api.dependencies import get_optional_tenant_context
...
from threat_report_intel.infrastructure.container import ThreatReportIntelContainer
...
def _apply_threat_report_intel_migrations() -> None:
...
async def tr_api_engine() -> AsyncIterator[AsyncEngine]:
...
@pytest_asyncio.fixture
async def tr_api_session_factory(
    tr_api_engine: AsyncEngine,
) -> async_sessionmaker[AsyncSession]:
    return async_sessionmaker(tr_api_engine, expire_on_commit=False)
...
def _build_app(
    ...
    *,
    ...
    register_threat_report_intel_exception_handlers(application)
    application.include_router(threat_report_intel_router, prefix="/api/v1")
    application.state.threat_report_intel_container = ThreatReportIntelContainer(
    ...
    def override_tenant_context() -> TenantContext:
        return TenantContext(
            user_id=str(user_id),
            email="threat-report-intel-test@example.com",
...
async def owner_client(
    tr_api_session_factory: async_sessionmaker[AsyncSession],
    organization_id: EntityId,
    user_id: EntityId,
) -> AsyncIterator[AsyncClient]:
    async for client in _client_for(
        tr_api_session_factory,
...
async def platform_admin_client(
    ...
    `PLATFORM_THREAT_REPORT_READ`/`_MANAGE` via `PlatformContext`, but
