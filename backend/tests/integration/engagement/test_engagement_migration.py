"""Alembic migration round-trip for M29 Phase 1 engagement (0063)."""
        ...
        async with engine.connect() as conn:
            result = await conn.execute(
                text(
                    ...
                    "WHERE table_schema = 'engagement' AND table_name = :t"
                ),
                {"t": table},
            )
...
def test_engagement_migration_0063_round_trip() -> None:
    """Sync test — Alembic env uses asyncio.run and cannot nest under pytest-asyncio."""
    if not os.environ.get("TEST_DATABASE_URL"):
        pytest.skip("TEST_DATABASE_URL not set")

    # Walk from current head back to parent, then upgrade (shared DB safe).
    _run_alembic("head")
    _run_alembic("downgrade:0062")
    _run_alembic("0063")
    assert asyncio.run(_alembic_head()) == "0063"
    assert asyncio.run(_schema_exists())
    for table in _ENGAGEMENT_TABLES:
        assert asyncio.run(_table_exists(table)), f"expected {table} after upgrade to 0063"
    ...
    assert not asyncio.run(_schema_exists()), (
        "expected engagement schema dropped after downgrade to 0062"
    )

    _run_alembic("0063")
    assert asyncio.run(_alembic_head()) == "0063"
    for table in _ENGAGEMENT_TABLES:
        assert asyncio.run(_table_exists(table)), f"expected {table} after re-upgrade to 0063"
