"""Alembic migration round-trip for M28 Phase 3 (0060)."""
...
def test_detection_migration_0060_round_trip() -> None:
    if not os.environ.get("TEST_DATABASE_URL"):
        pytest.skip("TEST_DATABASE_URL not set")

    _run_alembic("0060")
    assert asyncio.run(_alembic_head()) == "0060"
    assert asyncio.run(_table_exists("detection_executions"))
