"""Alembic migration round-trip for M29 Phase 1 operator (0064)."""
        ...
        async with engine.connect() as conn:
            result = await conn.execute(
                text(
                    ...
                    "WHERE table_schema = 'operator' AND table_name = :t"
                ),
                {"t": table},
            )
...
def test_operator_migration_0064_round_trip() -> None:
    if not os.environ.get("TEST_DATABASE_URL"):
        pytest.skip("TEST_DATABASE_URL not set")

    _run_alembic("head")
    _run_alembic("downgrade:0063")
    _run_alembic("0064")
    assert asyncio.run(_alembic_head()) == "0064"
    assert asyncio.run(_schema_exists())
    for table in _OPERATOR_TABLES:
        assert asyncio.run(_table_exists(table)), f"expected {table} after upgrade to 0064"
    ...
    assert not asyncio.run(_schema_exists()), (
        "expected operator schema dropped after downgrade to 0063"
    )

    _run_alembic("0064")
    assert asyncio.run(_alembic_head()) == "0064"
    for table in _OPERATOR_TABLES:
        assert asyncio.run(_table_exists(table)), f"expected {table} after re-upgrade to 0064"
