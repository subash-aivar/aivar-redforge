...
def test_application_service_does_not_import_orm_or_concrete_adapters() -> None:
    service_file = ROOT / "application" / "services" / "tool_application_service.py"
...
def test_no_acl_identity_port_exists() -> None:
    """An adversary tool's canonical_name has no upstream catalog — no
    identity ACL port should exist here (same call campaign_intel and
    malware_intel made)."""
