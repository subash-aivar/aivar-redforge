"""FastAPI fixtures for tool_intel API integration tests — mirrors
`tests/campaign_intel/api/conftest.py`'s shape exactly: a real ASGI app
with the tool_intel router mounted, `TenantContext`/
...
from tool_intel.api.dependencies import get_optional_tenant_context
from tool_intel.api.exception_handlers import register_tool_intel_exception_handlers
...
def _apply_tool_intel_migrations() -> None:
...
async def ti_api_engine() -> AsyncIterator[AsyncEngine]:
...
@pytest_asyncio.fixture
async def ti_api_session_factory(
    ti_api_engine: AsyncEngine,
) -> async_sessionmaker[AsyncSession]:
    return async_sessionmaker(ti_api_engine, expire_on_commit=False)
...
def _build_app(
    ...
    *,
    ...
    register_tool_intel_exception_handlers(application)
    application.include_router(tool_intel_router, prefix="/api/v1")
    application.state.tool_intel_container = ToolIntelContainer(session_factory=session_factory)

    def override_tenant_context() -> TenantContext:
        return TenantContext(
            user_id=str(user_id),
            email="tool-intel-test@example.com",
...
async def owner_client(
    ti_api_session_factory: async_sessionmaker[AsyncSession],
    organization_id: EntityId,
    user_id: EntityId,
) -> AsyncIterator[AsyncClient]:
    async for client in _client_for(
        ti_api_session_factory,
...
async def platform_admin_client(
    ...
    `PLATFORM_TOOL_READ`/`_MANAGE` via `PlatformContext`, but
