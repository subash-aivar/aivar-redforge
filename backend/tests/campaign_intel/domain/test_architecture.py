...
_FORBIDDEN_REDTEAM_CAMPAIGN_IMPORT = re.compile(
    r"^\s*(from|import)\s+(campaign|campaignexecution)\b", re.M
)
...
def test_no_intelligence_relationships_import_anywhere() -> None:
    ...
    externally (its `CAMPAIGN` entity type references a Campaign by
    opaque `entity_id`) — this context never imports its domain classes,
    and never duplicates its capability with a relationship model of its
    own."""
    for path in ROOT.rglob("*.py"):
        text = path.read_text()
...
def test_no_redteam_campaign_orchestration_import_anywhere() -> None:
    """`src/campaign` and `src/campaignexecution` model RedForge's OWN
    red-team campaign orchestration — an entirely different concept from
    a threat-intelligence record of a real-world adversary campaign.
    campaign_intel never couples to them."""
    for path in ROOT.rglob("*.py"):
        text = path.read_text()
        if _FORBIDDEN_REDTEAM_CAMPAIGN_IMPORT.search(text):
            raise AssertionError(f"{path} imports the red-team campaign context directly")


def test_no_relationship_model_defined_in_this_context() -> None:
    """The deliberate non-duplication decision, enforced: campaign_intel
    owns no relationship aggregate, VO, port or table."""
    for path in ROOT.rglob("*.py"):
        text = path.read_text()
...
def test_two_status_axes_are_separate_fields_on_the_aggregate() -> None:
    """`status` (real-world campaign) and `lifecycle_status` (RedForge
    record) must remain two distinct slots — collapsing them would erase
    the domain guarantee this context is built on."""
    from campaign_intel.domain.aggregates.campaign import Campaign

    assert "status" in Campaign.__slots__
    assert "lifecycle_status" in Campaign.__slots__
