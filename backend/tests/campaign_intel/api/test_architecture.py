"""Architecture/security-scan tests for the campaign_intel API layer,
mirroring `tests/malware_intel/api/test_architecture.py`'s
convention."""
...
_ORM_IMPORT = re.compile(
    r"^\s*(from|import)\s+campaign_intel\.infrastructure\.persistence\.(models|repositories)\b",
    re.M,
)
...
def test_routes_never_accept_tenant_id_from_request_body() -> None:
    from campaign_intel.api.schemas import campaign_schemas
    ...
    for name in request_schema_names:
        schema_cls = getattr(campaign_schemas, name)
...
def test_explicit_observation_routes_use_the_correct_gate() -> None:
    routes_text = (API_ROOT / "v1" / "campaigns.py").read_text()
    tenant_obs = re.search(r"async def observe_tenant_campaign\(.*?\n\) -> ", routes_text, re.S)
    global_obs = re.search(r"async def observe_global_campaign\(.*?\n\) -> ", routes_text, re.S)
...
def test_canonical_routes_have_no_static_permission_gate() -> None:
    """Every canonical `{campaign_id}` route derives authorization
    dynamically from the loaded Campaign's ownership scope
    ...
    for name in canonical_handlers:
        ...
        assert "_authorize_scope" in body or "OptionalTenantContextDep" in body, (
            f"{name} must derive authorization from loaded Campaign ownership scope"
        )


def test_public_route_count_matches_approved_canonical_contract() -> None:
    """16 total: 2 explicit observation routes + 2 unambiguous list
    routes + 12 canonical single `{campaign_id}` routes (get, aliases,
    objectives, regions, target-sectors, evidence-citations,
    source-attributions, transition-status, deprecate, revoke,
    supersede, reactivate)."""
    from redforge.app import create_app

    app = create_app()
    schema = app.openapi()
    cp_paths = [p for p in schema["paths"] if "/campaign-intel" in p]
    assert len(cp_paths) == 16, sorted(cp_paths)
...
def test_status_and_lifecycle_routes_are_distinct() -> None:
    """The two axes get their own routes — no single endpoint may move
    both."""
    routes_text = (API_ROOT / "v1" / "campaigns.py").read_text()
    assert "/{campaign_id}/transition-status" in routes_text
    for lifecycle_route in ("deprecate", "revoke", "supersede", "reactivate"):
        assert f'"/{{campaign_id}}/{lifecycle_route}"' in routes_text
