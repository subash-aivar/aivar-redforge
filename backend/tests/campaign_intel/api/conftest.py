"""FastAPI fixtures for campaign_intel API integration tests — mirrors
`tests/malware_intel/api/conftest.py`'s shape exactly: a real ASGI app
with the campaign_intel router mounted, `TenantContext`/
`PlatformContext` overridden via FastAPI's `dependency_overrides`
(never a spoofable request header), and a real Postgres-backed
`CampaignIntelContainer`."""
...
from campaign_intel.api.dependencies import get_optional_tenant_context
from campaign_intel.api.exception_handlers import register_campaign_intel_exception_handlers
...
def _apply_campaign_intel_migrations() -> None:
...
async def cp_api_engine() -> AsyncIterator[AsyncEngine]:
...
@pytest_asyncio.fixture
async def cp_api_session_factory(
    cp_api_engine: AsyncEngine,
) -> async_sessionmaker[AsyncSession]:
    return async_sessionmaker(cp_api_engine, expire_on_commit=False)
...
def _build_app(
    ...
    *,
    ...
    register_campaign_intel_exception_handlers(application)
    application.include_router(campaign_intel_router, prefix="/api/v1")
    application.state.campaign_intel_container = CampaignIntelContainer(
    ...
    def override_tenant_context() -> TenantContext:
        return TenantContext(
            user_id=str(user_id),
            email="campaign-intel-test@example.com",
...
async def owner_client(
    cp_api_session_factory: async_sessionmaker[AsyncSession],
    organization_id: EntityId,
    user_id: EntityId,
) -> AsyncIterator[AsyncClient]:
    async for client in _client_for(
        cp_api_session_factory,
...
async def platform_admin_client(
    ...
    `PLATFORM_CAMPAIGN_READ`/`_MANAGE` via `PlatformContext`, but
