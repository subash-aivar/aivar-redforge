"""PostgreSQL fixtures for risk_engine infrastructure integration
tests — matches `tests/credential_vault/infrastructure/conftest.py`'s
async fixtures exactly (`AsyncSession`/`asyncpg`, no synchronous
driver). Converted from a sync-`Session`/`psycopg` implementation to
this async form in the M48C contract correction — see
...
def apply_risk_engine_migrations() -> None:
...
@pytest_asyncio.fixture
async def re_session_factory(
    re_engine: AsyncEngine,
...
def re_uow_factory(
    ...
    return make_sqlalchemy_uow_factory(re_session_factory)


@pytest_asyncio.fixture
async def re_session(
    re_session_factory: async_sessionmaker[AsyncSession],
) -> AsyncIterator[AsyncSession]:
    session = re_session_factory()
    yield session
