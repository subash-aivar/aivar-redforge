"""FastAPI fixtures for intelligence_relationships API integration
tests — mirrors `tests/attack_pattern_intel/api/conftest.py`'s shape
exactly: a real ASGI app with the intelligence_relationships router
mounted, `TenantContext`/`PlatformContext` overridden via FastAPI's
`dependency_overrides` (never a spoofable request header), and a real
Postgres-backed `IntelligenceRelationshipsContainer`."""
...
from intelligence_relationships.api.dependencies import get_optional_tenant_context
...
def _apply_intelligence_relationships_migrations() -> None:
...
async def ir_api_engine() -> AsyncIterator[AsyncEngine]:
...
@pytest_asyncio.fixture
async def ir_api_session_factory(
    ir_api_engine: AsyncEngine,
) -> async_sessionmaker[AsyncSession]:
    return async_sessionmaker(ir_api_engine, expire_on_commit=False)
...
def _build_app(
    ...
    *,
    ...
    register_intelligence_relationships_exception_handlers(application)
    application.include_router(intelligence_relationships_router, prefix="/api/v1")
    application.state.intelligence_relationships_container = IntelligenceRelationshipsContainer(
    ...
    def override_tenant_context() -> TenantContext:
        return TenantContext(
            user_id=str(user_id),
            email="intelligence-relationships-test@example.com",
...
async def owner_client(
    ir_api_session_factory: async_sessionmaker[AsyncSession],
    organization_id: EntityId,
    user_id: EntityId,
) -> AsyncIterator[AsyncClient]:
    async for client in _client_for(
        ir_api_session_factory,
...
async def platform_admin_client(
    ...
    `PLATFORM_RELATIONSHIP_READ`/`_MANAGE` via `PlatformContext`, but
