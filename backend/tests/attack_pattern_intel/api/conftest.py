"""FastAPI fixtures for attack_pattern_intel API integration tests —
mirrors `tests/ioc_intelligence/api/conftest.py`'s shape exactly: a
real ASGI app with the attack_pattern_intel router mounted,
`TenantContext`/`PlatformContext` overridden via FastAPI's
`dependency_overrides` (never a spoofable request header), and a real
Postgres-backed `AttackPatternIntelContainer`."""
...
from attack_pattern_intel.api.dependencies import get_optional_tenant_context
...
def _apply_attack_pattern_intel_migrations() -> None:
...
async def ap_api_engine() -> AsyncIterator[AsyncEngine]:
...
@pytest_asyncio.fixture
async def ap_api_session_factory(
    ap_api_engine: AsyncEngine,
) -> async_sessionmaker[AsyncSession]:
    return async_sessionmaker(ap_api_engine, expire_on_commit=False)
...
def _build_app(
    ...
    *,
    ...
    register_attack_pattern_intel_exception_handlers(application)
    application.include_router(attack_pattern_intel_router, prefix="/api/v1")
    application.state.attack_pattern_intel_container = AttackPatternIntelContainer(
    ...
    def override_tenant_context() -> TenantContext:
        return TenantContext(
            user_id=str(user_id),
            email="attack-pattern-intel-test@example.com",
...
async def owner_client(
    ap_api_session_factory: async_sessionmaker[AsyncSession],
    organization_id: EntityId,
    user_id: EntityId,
) -> AsyncIterator[AsyncClient]:
    async for client in _client_for(
        ap_api_session_factory,
...
async def platform_admin_client(
    ...
    `PLATFORM_ATTACK_PATTERN_READ`/`_MANAGE` via `PlatformContext`, but
