"""FastAPI dependencies for tool_intel routes.
Follows `campaign_intel.api.dependencies`'s shape exactly: a container
...
# `campaign_intel.api.dependencies`'s identical precedent.
_bearer_scheme = HTTPBearer(auto_error=False)


async def get_tool_intel_container(request: Request) -> ToolIntelContainer:
    container: ToolIntelContainer = request.app.state.tool_intel_container
    return container


async def get_tool_intel_session(
    container: ToolIntelContainer = Depends(get_tool_intel_container),
...
async def get_tool_service(
    container: ToolIntelContainer = Depends(get_tool_intel_container),
    session: AsyncSession = Depends(get_tool_intel_session),
) -> ToolApplicationService:
    return container.build_service(session)


TenantIdDep = Annotated[EntityId, Depends(get_tenant_uuid)]
ToolServiceDep = Annotated[ToolApplicationService, Depends(get_tool_service)]
