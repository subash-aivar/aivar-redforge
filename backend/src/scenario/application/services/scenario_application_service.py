"""ScenarioApplicationService — create, publish, deprecate, instantiate, list."""
...
def _to_template_dto(template: ScenarioTemplate) -> ScenarioTemplateDTO:
    return ScenarioTemplateDTO(
        ...
        threat_actor_name=(
            template.threat_actor_ref.name if template.threat_actor_ref else None
        ),
        covered_techniques=[
            {
...
class ScenarioApplicationService:
    """Orchestrates scenario template lifecycle and instantiation."""
    ...
    async def _write_graph(self, template: ScenarioTemplate) -> None:
        ...
        if (
            template.threat_actor_ref is not None
            and template.threat_actor_ref.threat_actor_id
        ):
    ...
    async def deprecate(
        self, cmd: DeprecateScenarioTemplateCommand
    ) -> ScenarioTemplateDTO:
        tenant_id = TenantId(cmd.tenant_id)
        template_id = ScenarioTemplateId(cmd.template_id)
        now = datetime.now(UTC)
    ...
    async def instantiate(
        self, cmd: InstantiateScenarioCommand
    ) -> InstantiationResultDTO:
        """Produce campaign/task graph draft specs — caller feeds into campaign context."""
        tenant_id = TenantId(cmd.tenant_id)
        template_id = ScenarioTemplateId(cmd.template_id)
        now = datetime.now(UTC)
    ...
    async def list_published(
        self, query: ListPublishedScenariosQuery
    ) -> list[ScenarioTemplateDTO]:
