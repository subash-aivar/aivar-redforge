"""FastAPI dependencies for threat_report_intel routes.
Follows `infrastructure_intel.api.dependencies`'s shape exactly: a
container pulled off `request.app.state`, tenant identity resolved only
from the signed access token via `redforge.api.security` — `tenant_id`
is NEVER accepted from a request body field, only derived here."""
...
# `infrastructure_intel.api.dependencies`'s identical precedent.
_bearer_scheme = HTTPBearer(auto_error=False)


async def get_threat_report_intel_container(request: Request) -> ThreatReportIntelContainer:
    container: ThreatReportIntelContainer = request.app.state.threat_report_intel_container
    return container


async def get_threat_report_intel_session(
    container: ThreatReportIntelContainer = Depends(get_threat_report_intel_container),
...
async def get_threat_report_service(
    container: ThreatReportIntelContainer = Depends(get_threat_report_intel_container),
    session: AsyncSession = Depends(get_threat_report_intel_session),
) -> ThreatReportApplicationService:
    return container.build_service(session)


TenantIdDep = Annotated[EntityId, Depends(get_tenant_uuid)]
ThreatReportServiceDep = Annotated[
    ThreatReportApplicationService, Depends(get_threat_report_service)
]
