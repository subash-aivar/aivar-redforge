"""FastAPI dependencies for infrastructure_intel routes.
Follows `tool_intel.api.dependencies`'s shape exactly: a container
...
# `tool_intel.api.dependencies`'s identical precedent.
_bearer_scheme = HTTPBearer(auto_error=False)


async def get_infrastructure_intel_container(request: Request) -> InfrastructureIntelContainer:
    container: InfrastructureIntelContainer = request.app.state.infrastructure_intel_container
    return container


async def get_infrastructure_intel_session(
    container: InfrastructureIntelContainer = Depends(get_infrastructure_intel_container),
...
async def get_infrastructure_service(
    container: InfrastructureIntelContainer = Depends(get_infrastructure_intel_container),
    session: AsyncSession = Depends(get_infrastructure_intel_session),
) -> InfrastructureApplicationService:
    return container.build_service(session)


TenantIdDep = Annotated[EntityId, Depends(get_tenant_uuid)]
InfrastructureServiceDep = Annotated[
    InfrastructureApplicationService, Depends(get_infrastructure_service)
]
