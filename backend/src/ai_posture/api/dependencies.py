"""FastAPI dependencies for ai_posture."""
...
def get_container() -> AIPostureContainer:
    global _container
    if _container is None:
        _container = AIPostureContainer()
    return _container
...
async def get_tenant_id(tenant: TenantContext = Depends(get_tenant_context)) -> EntityId:
    ...
    and read/write that org's AI posture data. get_tenant_context
