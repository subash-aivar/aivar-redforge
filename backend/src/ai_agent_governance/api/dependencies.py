...
_container: AgentGovernanceContainer | None = None


def get_container() -> AgentGovernanceContainer:
    global _container
    if _container is None:
        _container = AgentGovernanceContainer()
    return _container
...
async def get_tenant_id(tenant: TenantContext = Depends(get_tenant_context)) -> UUID:
    ...
    and read/write that org's AI agent governance data.
