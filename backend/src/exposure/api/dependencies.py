...
_container: ExposureContainer | None = None


def get_container() -> ExposureContainer:
    global _container
    if _container is None:
        _container = ExposureContainer()
    return _container
...
async def get_tenant_id(tenant: TenantContext = Depends(get_tenant_context)) -> EntityId:
    ...
    and read/write that org's exposure data. get_tenant_context verifies
