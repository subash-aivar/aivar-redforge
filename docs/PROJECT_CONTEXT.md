...
|--------|------------------------|------------------|-------|
| M22 Phase 4 + Phase 5 (COMPLETE — Phases 6-7 not started) | Threat Fusion + Attack Path Engine | Phase 4 Threat Fusion bounded context consumes **only** Phase 1 reference-data catalog rows (populated by Phase 3 STIX/TAXII / Phase 1 admin loads) — no STIX re-parse, no M18 IP enrichment. `FusedIndicator` aggregate owns lifecycle, temporal validity, multi-source attributions, and `AggregatedRisk` (explicit `NO_EVIDENCE` sentinel); `FusedRelationship` is a separate entity (not an unbounded in-aggregate collection). `FusionConflictPolicy` + documented `DEFAULT_FUSION_WEIGHTS` + admin overrides in `threat_intel_fusion_config`; distinct `FusionConfidence` ladder (≠ M21). Migration `0037`: `fused_indicators`, `fused_indicator_sources`, `fused_relationships`, `threat_intel_fusion_config`. Phase 5 Attack Path Engine builds an ATT&CK relationship graph from fused outputs only (does not duplicate fusion logic); BFS path discovery under `PathExplosionBudget` (max_depth=6, max_fan_out=4) with cycle skip; `InferenceGatePolicy` for INFERRED hops; weakest-link `PathConfidence` (≠ FusionConfidence); risk/exposure/kill-chain propagation. `AttackPath` aggregate holds summary only — steps + evidence via child tables (`attack_path_steps`, `attack_path_step_evidence`). Migration `0038`. Ontology v7 additive node/edge kinds. Internal APIs `/threat-intel/fusion/*` and `/attack-paths/*` with platform permissions + audit (`INDICATOR_FUSED`, `FUSION_WEIGHT_UPDATED`, `ATTACK_PATH_COMPUTED`, `ATTACK_PATH_UPDATED`). Tests: 9 fusion domain + 9 attack-path domain + 3 PG integration. Ruff/Mypy clean on Phase 4/5 modules. See `docs/M22_PHASE4_PHASE5_THREAT_FUSION_ATTACK_PATH_REPORT.md`. |
| M22 Phase 3 (COMPLETE — Phases 4-5 now complete; 6-7 not started) | STIX/TAXII Integration | First real feed connector for the Phase 2 platform — a TAXII 2.1 client + STIX 2.1 parser/mapper plugging into Phase 2's `FeedSyncExecutor`/`FeedConnectorRegistry` seam with **zero changes** to Phase 2's orchestration/locking/retry/scheduling code (the only Phase-2-owned edit is one `registry.register(...)` call added to `app.py`'s existing startup hook) and **no new migration** (Phase 1's 4 reference-data tables receive every mapped object via the existing, unmodified `ReferenceDataAdminService` — the only schema-adjacent change is one additive `ReferenceDataSource.STIX_TAXII_FEED` `StrEnum` member, since `source_system` is a plain `VARCHAR`, not a DB enum). Custom, lightweight STIX 2.1 parser (`domain/threat_intel/stix_parser.py`) — deliberately not the third-party `stix2` library, per the Hardening Review's P0 object-graph-explosion finding — validates raw byte length, defensively JSON-decodes (catching `RecursionError` on adversarial nesting), and enforces `MAX_OBJECTS_PER_CONTAINER`/`MAX_NESTING_DEPTH` caps before any per-object parsing; supports exactly 4 STIX object types (`attack-pattern`, `x-mitre-tactic`, `relationship`, `vulnerability`) via narrow, fixed-field parsers into flat `stix_objects.py` dataclasses — never a generic graph walk. `TaxiiClient` (`infrastructure/threat_intel/taxii_client.py`) implements TAXII 2.1 discovery/collections/paginated-object-retrieval with dynamic SSRF defense for admin-configurable endpoint URLs (per the Hardening Review's P0 SSRF finding, since the existing `ThreatIntelHttpClient`'s static host-allowlist pattern cannot apply to a per-feed URL): HTTPS-only scheme, DNS resolution + IP classification rejecting loopback/link-local/private/multicast/reserved/cloud-metadata ranges (reusing the existing `is_public_ip()` classifier, not a second implementation), no redirect-following, and response-size capping; Basic/Bearer auth resolved at execution time via the existing `CredentialResolverPort` — never read from `connector_config`. A three-layer Anti-Corruption Layer separates STIX wire format from the RedForge domain model per the Hardening Review's P1 finding (no ACL existed): domain-layer parser (STIX shape only) → application-layer mapper `stix_reference_data_mapper.py` (STIX vocabulary → Phase 1's `TacticInput`/`TechniqueInput`/`RelationshipInput`/`VulnerabilityInput` DTOs, including sub-technique parent resolution and tactic-shortname→ID resolution) → `StixTaxiiFeedConnector` (orchestrates fetch→parse→map→upsert, calling the unmodified `ReferenceDataAdminService` for every write). STIX `external_references[].url` values are treated as inert text metadata everywhere — never fetched — per the Hardening Review's P1 finding on attacker-influenced embedded URLs. Incremental sync reuses Phase 2's opaque `checkpoint` exactly as designed: the TAXII `added_after` filter is populated from the *attempt's own start time*, never a value echoed back by the remote server. Two module-level ceilings (`MAX_PAGES_PER_SYNC_ATTEMPT`, `MAX_TOTAL_OBJECTS_PER_SYNC_ATTEMPT`) bound every sync attempt regardless of server behavior. **One real defect found and fixed by this phase's own integration tests, not a review finding**: the connector's first-draft audit `actor_id` (`f"system:stix_taxii_connector:{feed_id}"`) overflowed `platform_audit_log.actor_id`'s `VARCHAR(26)` column (sized exactly for a ULID — confirmed by measuring Phase 2's own `_SCHEDULER_ACTOR_ID` constant, which is *exactly* 26 characters) the instant a 26-character `feed_id` ULID was appended to any prefix; fixed by using the bare `feed_id` as the actor identity (still a real, unique, directly-queryable audit identity), confined entirely to `stix_taxii_connector.py` with no Phase 1/2 code or schema touched. 138 new tests (46 domain — `StixId` shape, container/object-list size/count/depth caps, per-type parsing, dispatch; 18 application — tactic/technique/relationship/vulnerability mapping incl. sub-techniques and unmapped-item isolation; 33 infrastructure — TAXII SSRF gate incl. mocked-DNS private/loopback/metadata rejection, auth, discovery, collections, pagination, transport failures; 41 application — config resolution, auth building, API-root resolution, fetch/parse/map/upsert flow incl. ceilings, full `execute()` against fakes, registry integration) + 8 PostgreSQL integration tests (real `StixTaxiiFeedConnector` + real `TaxiiClient` against `httpx.MockTransport` + real `FeedSyncOrchestrationService`: end-to-end batch persistence, incremental-sync checkpoint provenance verified against the actual outgoing request, idempotent re-sync convergence, unreadable-collection and real-DNS-resolver SSRF error handling, Bearer-credential resolution/forwarding and missing-credential rejection, and confirmation that every other `FeedSourceKind` still fails honestly with `UnknownFeedConnectorError`). Full M22 Phase 2 suite (33 tests) and full M22 Phase 1 suite (39 tests) re-verified passing unchanged, confirming zero regression to either pre-existing bounded context. Clean `alembic upgrade head`/`downgrade`/`upgrade` round-trip re-confirmed (no new migration to verify). Ruff clean, mypy clean (0 errors in 789 source files; 1 pre-existing unrelated `maxminddb` stub gap disclosed). See [M22_PHASE3_STIX_TAXII_INTEGRATION_REPORT.md](M22_PHASE3_STIX_TAXII_INTEGRATION_REPORT.md). | 138 new (46 domain + 18 + 33 + 41 application/infrastructure) + 8 PG integration (M22 Phase 3) + 33 PG integration (M22 Phase 2 regression) + 39 PG integration (M22 Phase 1 regression) |
| M22 Phase 2 (COMPLETE — Phases 3-7 not started) | Feed Synchronization Foundation | Generic, provider-agnostic feed-sync platform for future STIX/TAXII connectors — no STIX/TAXII/Fusion/Attack-Path/Investigation-integration/frontend code written this phase. Two aggregate roots: `Feed` (registration, configuration, lifecycle `draft→active⇄paused→disabled`, scheduling state, checkpoint, consecutive-failure counter) and `FeedSyncRun` (one execution's `pending→running→succeeded/failed` history), linked only by `feed_id` — kept separate so `FeedSyncRun`'s unbounded execution history never has to load inside the `Feed` aggregate. Value objects: `FeedKey` (lowercase snake_case, 3-64 chars), `SyncSchedule` (interval-seconds, `MIN_SYNC_INTERVAL_SECONDS` floor), `RetryPolicy` (max attempts, base/max delay, jitter factor). Global-vs-tenant scoping reuses Phase 1's exact `IngestionScope` + `CHECK` constraint + dual-partial-unique-index pattern verbatim, so `feeds.feed_key` never collides between the global and tenant namespaces. Concurrency is two-layered: a per-feed `pg_advisory_xact_lock` narrows the race window during `FeedSyncOrchestrationService.trigger_sync`, but the actual correctness guarantee is a database-level partial unique index (`ux_fsr_feed_active_run` on `feed_sync_runs.feed_id WHERE status IN ('pending','running')`) — proven directly with a genuine 10-way `asyncio.gather()` concurrent-trigger race in the PostgreSQL integration suite (exactly one caller succeeds, nine observe `FeedSyncAlreadyRunningError`, never a second run row). The orchestration service runs as three separate short-lived transactions (`_start_run` → connector call (outside any transaction) → `_finalize_run`) so a slow external feed call never holds the per-feed advisory lock; retries between those transactions reuse the existing `RetryConfig`/`RetryExecutor` from `application.platform.retry_strategy` verbatim (no new backoff implementation). A background `FeedSyncSchedulerWorker` polls `list_due_for_sync` (backed by a partial index on `feeds.next_sync_due_at WHERE status='active'`), coordinated across replicas by a non-blocking global `pg_try_advisory_xact_lock` (mirrors the existing NDR/DDoS worker coordination pattern) — separate from the per-feed lock used during an individual sync. Extensibility seam for Phase 3: `FeedSyncExecutor` (a `Protocol`) + `FeedConnectorRegistry` keyed by the closed, deliberately generic `FeedSourceKind` enum (`static_http_download`/`http_api_incremental`/`manual_upload` — no STIX/TAXII-named values yet); the registry is initialized **empty** in production wiring (`runtime_container.py`), so every `POST /feeds/{id}/sync` in this phase honestly returns `422 UnknownFeedConnectorError` rather than a silent success — the only `FeedSyncExecutor` implementation anywhere in the repository is a test-only stub confined to the PostgreSQL integration test module, used solely to prove the plug-in seam and retry/locking behavior end-to-end. Migration 0036 creates 2 tables, schema-only: `feeds` (scope/org `CHECK` constraint + 2 partial unique key indexes + partial due-for-sync index) and `feed_sync_runs` (cascade FK to `feeds`, partial unique active-run index). `InvalidFeedStatusTransitionError` classified as `ConflictError` (409, not 422) for consistency with `FeedNotActiveError`/`FeedSyncAlreadyRunningError`. Application layer: `FeedAdminService` (register/activate/pause/disable/update-configuration, audit-logged via 6 new `AuditAction`s) + `FeedQueryService` (read-only) + `FeedSyncOrchestrationService`. API: new internal administration router (`api/v1/feed_sync.py`) gated by two new platform permissions (`PLATFORM_FEED_SYNC_READ`/`PLATFORM_FEED_SYNC_MANAGE`, wired into `SECURITY_ADMIN`/`AUDITOR` roles) — no public Threat Intelligence API surface added. `_EXPECTED_MIGRATION_HEAD` bumped 0035→0036; pre-existing hardcoded-head assertions updated to match (`test_sprint29_replay_pipeline.py` ×2, `test_startup_validator.py`, and the M22 Phase 1 proof-database's own `test_migration_head_is_0035` renamed to `test_migration_head_is_current` and re-pinned to the moving head rather than a historical literal, since that proof database is migrated with `alembic upgrade head`). 71 domain unit tests (value objects, both aggregates' full lifecycles, exceptions) + 33 PostgreSQL integration tests (migration structure, both repositories' full behavior including the DB-level scope/org-pairing rejection and active-run uniqueness backstop, full orchestration lifecycle — success/transient-failure-then-retry-success/permanent-failure/non-active-feed rejection/unknown-connector rejection/the 10-way concurrency race — and real-HTTP API acceptance with platform-permission RBAC). Full M22 Phase 1 PostgreSQL suite (39 tests) and full M21 PostgreSQL suite (51 tests) re-verified passing unchanged against the post-0036 schema, confirming zero regression to either pre-existing bounded context. Ruff clean, mypy clean (0 errors in 782 source files). See [M22_PHASE2_FEED_SYNCHRONIZATION_FOUNDATION_REPORT.md](M22_PHASE2_FEED_SYNCHRONIZATION_FOUNDATION_REPORT.md). | 71 domain + 33 PG integration (M22 Phase 2) + 39 PG integration (M22 Phase 1 regression) + 51 PG integration (M21 regression) |
| M22 Phase 1 (COMPLETE — Phases 3-7 not started) | Threat Intelligence Foundation: ATT&CK Framework + CVE/KEV Global Reference Data | Repository-first implementation following the authoritative `docs/architecture/m22/m22_architecture_freeze.html` spec, corrected per `docs/architecture/m22/m22_hardening_review.html`'s P0/P1 findings. New `domain/threat_intel/` reference-data slice added alongside the existing M18 threat-intel VOs/services (no collision, no rename). **Key architectural correction from the hardening review**: reference data (`AttackTactic`, `AttackTechnique`, `AttackTechniqueRelationship`, `Vulnerability`) modeled as immutable **entities**, not aggregates — global, read-only catalog data with identity but no lifecycle/invariants of their own; the one genuine aggregate is `ReferenceDataIngestionRecord`, tracking idempotent ingestion facts with an `IngestionScope` (GLOBAL/TENANT) discriminator and an `InvalidIngestionScopeError` guard preventing `organization_id` on GLOBAL-scope records (and requiring it on TENANT-scope ones). Value objects: `TechniqueId`/`TacticId` (`T####`/`TA####` MITRE ID shape validation), `CveId` (`CVE-YYYY-NNNN+` shape), `EpssScore`/`CvssScore` (bounded 0.0-1.0 / 0.0-10.0, closed CVSS version enum). Migration 0035 creates 5 tables, schema-only (no bulk data inserts — seeding is an explicit application-layer concern per the hardening review's P1 finding): `attack_tactics`, `attack_techniques` (self-referencing `parent_technique_id`, GIN full-text index on `name`), `attack_technique_relationships` (self-referencing technique pairs), `vulnerabilities` (CVSS v3/v2, EPSS, CISA KEV columns), `stix_ingestion_log` (scope-discriminated idempotency ledger — a `CHECK` constraint enforces `organization_id IS NULL` for GLOBAL scope, with two mutually-exclusive partial unique indexes so global and future tenant-scoped STIX ingestion never conflate identity). All 5 global reference tables deliberately carry **no** `organization_id` column (the hardening review's P0 finding) — tenant isolation is structurally not applicable since the catalog itself is platform-wide truth, not tenant-owned data. Self-referencing foreign keys declared `deferrable=True, initially="DEFERRED"` (migration + ORM model, consistent) so bulk technique/relationship upserts can insert parent-before-child or child-before-parent within one transaction. Repository implementations follow the M18/M21 established `begin_nested()` SAVEPOINT + `IntegrityError` refetch-and-converge upsert pattern; `search_by_name` uses `to_tsvector`/`to_tsquery` full-text search with input sanitization (non-alphanumeric stripped, empty-after-sanitization short-circuits to an empty result rather than a malformed query). Application layer: `ReferenceDataAdminService` (idempotent batch upsert commands, audit-logged via a new `REFERENCE_DATA_INGESTED` `AuditAction`) + `ReferenceDataQueryService` (read-only catalog queries) — CRUD-level only, no STIX/TAXII/Fusion/Attack-Path orchestration (explicitly out of Phase 1 scope). API: new internal administration router (`api/v1/threat_intel_reference_data.py`) gated by two new platform-level permissions (`PLATFORM_THREAT_INTEL_READ`/`PLATFORM_THREAT_INTEL_MANAGE`, wired into `SECURITY_ADMIN`/`AUDITOR` platform roles) — no public Threat Intelligence API surface added. `_EXPECTED_MIGRATION_HEAD` bumped 0034→0035; 3 pre-existing tests with a hardcoded `"0034"` expected-head literal (`test_sprint29_replay_pipeline.py` ×2, `test_startup_validator.py`, plus the M21 proof-database's own `test_migration_head_is_0034` renamed to `test_migration_head_is_0035`) updated to match. 75 domain unit tests (value objects, entities, aggregate invariants, exceptions) + 39 PostgreSQL integration tests (migration up/down/up-reversibility, all 5 repository upsert/search/idempotency paths including a real concurrent-upsert convergence proof, `ReferenceDataIngestionRecord`'s GLOBAL-scope `organization_id` invariant, application-service batch ingestion + audit trail, and real-HTTP API acceptance with platform-permission RBAC). Full M21 PostgreSQL suite (51 tests) re-verified passing unchanged against the post-0035 schema, confirming no regression to the existing investigation bounded context. Ruff clean, mypy clean (0 errors introduced; 1 pre-existing unrelated `maxminddb` stub gap disclosed, not in scope). Explicitly NOT implemented this phase (deferred to M22 Phases 2-7 per the approved spec): STIX/TAXII ingestion, Fusion Engine, Attack Path Engine, background workers, frontend, Investigation-context integration. **Post-implementation review-fix pass** (self-directed audit against the hardening review, no external findings list provided) found and fixed 3 real defects in `ReferenceDataAdminService`, none touching the migration, API surface, aggregate boundaries, or authorization model: (1) the ingestion-log write happened *before* the batch item's domain entity was constructed/validated, so a validation failure still left a "successfully ingested" ledger row — permanently poisoning that `external_id` against any future retry with the same (still-invalid) payload, which would hash-match the poisoned record and be silently skipped as `unchanged` forever; fixed by deferring the ingestion-log write (`_record_ingestion`) until after the corresponding repository upsert has already succeeded, with a read-only pre-check (`_check_unchanged`) run first. (2) `parent_technique_id` and relationship technique-side foreign keys are `DEFERRABLE INITIALLY DEFERRED` (by design, so batches may insert children before parents) — meaning a genuinely dangling reference was only caught at `uow.commit()`, an unhandled `IntegrityError` *outside* the per-item try/except that silently aborted the entire batch (including every otherwise-valid item), contradicting the batch-isolation guarantee the service already advertised and tested for format-validation failures; fixed with an application-layer pre-check (`AttackTechniqueRepository.list_all_ids()`/`AttackTacticRepository.list_all_ids()`, new read-only repository methods) that resolves references against already-stored rows unioned with the current batch (for `parent_technique_id`, which may resolve within the same call) before ever attempting the write, raising the previously-unused `UnknownTechniqueReferenceError`/`UnknownTacticReferenceError` domain exceptions as an isolated per-item failure instead. (3) two items in the same batch sharing an `external_id` with *conflicting* content silently resolved via last-write-wins with no signal to the caller; fixed by wiring up the previously-unused `DuplicateIngestionError` via an in-memory per-batch hash tracker (`_check_duplicate_in_batch`) — an identical repeated item is still a harmless no-op (resolves to `unchanged`), only a content conflict on a repeated `external_id` is now rejected. Also removed one dead ORM convenience method (`StixIngestionLogModel.as_dict()`, unused anywhere, with an inaccurate docstring claiming query-service usage) and corrected an inaccurate `IngestionScope`/exception-name reference in this very milestone's own documentation. 7 new PostgreSQL integration regression tests cover all three fixes plus the two now-wired reference exceptions. | 75 domain + 39 PG integration (M22) + 51 PG integration (M21 regression) |
| M21 (COMPLETE) | Cross-Domain Security Correlation & Unified Threat Investigation | New `domain/investigations/` bounded context. Correlation engine: R01/R03/R04/R06 deterministic rules; R02/R05/R07 stubs only. Canonical normalizers for IP, resource, detection entity. `InvestigationCase` aggregate root with optimistic concurrency (`version` column). Advisory-lock-safe case creation (`pg_advisory_xact_lock`). Partial unique index (`ux_inv_org_corr_active WHERE status != 'RESOLVED'`) — one active case per correlation_key. 24h reopen window. SAVEPOINT+refetch race safety identical to M19/M20. Migration 0034: `investigation_cases`, `investigation_evidence_links`, `investigation_events`, `correlation_cursors`. 9 REST endpoints under `/investigations/` (READ + MANAGE permissions). `SourceDomain.INVESTIGATION` wired into `fetch_merged_candidates()` operational stream. Security Graph ontology v6 adds `NodeKind.INVESTIGATION` + `EdgeKind.CORRELATED_WITH` enums; `SecurityGraphProjector.project_investigation()` implemented and wired into `correlate_pair()` (best-effort, non-blocking). Frontend: posture dashboard + case list + case detail + timeline + evidence + resolve modal. Browser acceptance: full lifecycle (acknowledge→start→resolve→hard refresh) with real OWNER user, no console errors, no API 500s. RBAC: ANALYST (read-only) denied MANAGE (403 proven); OWNER allowed MANAGE (200 proven); suspended membership/org denial proven against M21 endpoints. Audit: 4 actions wired (`INVESTIGATION_ACKNOWLEDGED`, `INVESTIGATION_STARTED`, `INVESTIGATION_RESOLVED`, `INVESTIGATION_EVIDENCE_ADDED`). ATM: 48 PROVEN, 1 PARTIALLY PROVEN (custom group — M17 mechanism), 1 NOT PROVEN (lab cleanup — terminal state, P3), 7 NOT APPLICABLE, 57 total. 153 tests (41 domain + 51 correlation + 51 PG integration + 10 ATM integrity). See [M21_CROSS_DOMAIN_CORRELATION_AND_UNIFIED_INVESTIGATION_REPORT.md](M21_CROSS_DOMAIN_CORRELATION_AND_UNIFIED_INVESTIGATION_REPORT.md), [M21_ADVERSARIAL_TRACEABILITY_MATRIX.md](M21_ADVERSARIAL_TRACEABILITY_MATRIX.md), [M21_COMPLETION_CHECKPOINT.md](M21_COMPLETION_CHECKPOINT.md). | 4,554 collectible |
| M20 (COMPLETE) | Advanced NDR, UEBA, and Behavioral Threat Detection | New `domain/behavior/` bounded context. Detection engine: 8 supported detections (`HIGH_FAN_OUT`, `PORT_SCAN_SUSPECTED`, `NEW_DESTINATION`, `RARE_DESTINATION`, `BEACONING_SUSPECTED`, `ABNORMAL_OUTBOUND_TRANSFER`, `UNUSUAL_EAST_WEST`, `UNUSUAL_SERVICE_ACCESS`) all derived from canonical `telemetry_events` (Zeek/Suricata) — no fabricated signals, no ML inference. 4 explicitly unsupported detections documented as NOT_APPLICABLE (user auth anomalies, impossible travel, failed connection ratio, lateral movement confirmed) — canonical data constraints honestly disclosed. p75 adaptive baseline (same pattern as M19 DDoS: `MIN_BASELINE_WINDOWS=10` before ESTABLISHED confidence, poisoning-resistant). Beaconing analysis: jitter_coefficient = std/mean ≤ 0.25, ≥ 8 samples, ≥ 20s interval — mandatory C2 caveat in `missing_evidence`. Race-safety: `pg_advisory_xact_lock` + partial unique index `ux_bd_org_corr_active` (one active detection per correlation_key) — identical to M19 pattern; 10-concurrent-session proof passes. New tables (migration 0033): `behavior_entity_baselines`, `behavior_observations`, `behavior_detections`, `behavior_detection_events`. 7 REST endpoints under `/behavior/`. 2 new permissions (`behavior:read`, `behavior:manage`). 5 frontend pages (`/behavior`, `/behavior/detections`, `/behavior/detections/[id]`, `/behavior/entities`, `/behavior/network`). All pages show honest cold-start/empty states — zero fabricated metrics. 36 domain unit tests + 26 PostgreSQL integration tests (tenancy, RBAC, concurrency proof, lab proofs A/B/C/D, worker runtime, operational stream integration, traceability consistency). ATM: 35 PROVEN, 4 NOT_APPLICABLE, 0 NOT_PROVEN. Migration head `0033`. Browser populated acceptance: 64 real detections, 5 entities, 48 network relationships — all from canonical telemetry_events (lab data removed post-acceptance). 6 surgical closure gaps fixed (startup validator migration head, SourceDomain.BEHAVIOR enum, list_detection_events_since, stream "W" source tag, pg_try_advisory_xact_lock check, traceability tests made consistency-only). See [M20_ADVANCED_NDR_UEBA_BEHAVIORAL_SECURITY_REPORT.md](M20_ADVANCED_NDR_UEBA_BEHAVIORAL_SECURITY_REPORT.md), [M20_ADVERSARIAL_TRACEABILITY_MATRIX.md](M20_ADVERSARIAL_TRACEABILITY_MATRIX.md), [M20_COMPLETION_CHECKPOINT.md](M20_COMPLETION_CHECKPOINT.md). | 4,413 passed / 4 pre-existing failures (network_security scope) / 5 skipped |
| M19 (COMPLETE) | Advanced DDoS Defense Center | New `domain/ddos/` bounded context (value objects: `IncidentStatus`, `DDoSClassification`, `DDoSSeverity`, `MitigationMode`, `BaselineConfidence`; detection engine: `compute_baseline()` p75 rolling history + `evaluate_window()` 7-signal deterministic evaluation — no ML, no fabricated signals). New `ddos_protected_resources`, `ddos_detection_policies`, `ddos_observation_windows`, `ddos_incidents`, `ddos_incident_events`, `ddos_mitigation_recommendations` tables (migration 0031). `DDoSDetectionWorker` asyncio poll worker wired into `GracefulShutdownCoordinator`. 18 REST endpoints under `/ddos`. 3 new permissions (`ddos:read`, `ddos:manage`, `ddos:mitigation_approve`) added to `ROLE_PERMISSIONS`. DDoS as 8th SSE stream source (tag "Z", try/except guarded). 6 frontend pages (`/ddos`, `/ddos/incidents`, `/ddos/incidents/[id]`, `/ddos/traffic`, `/ddos/protected-resources`, `/ddos/mitigation`). All visualizations from real `ddos_observation_windows` data — no hardcoded metrics. Evidence honesty: TCP flags documented absent from telemetry schema; SYN flood classified `_SUSPECTED` via Suricata signatures only; cold-start suppresses deviation signals when `< 20` baseline windows; detection suppressed when `event_count < 10`. Mitigation: `RECOMMEND_ONLY` default, `provider_type=None`, no automated execution in M19. Concurrency safety: SAVEPOINT+refetch for concurrent first-detection, optimistic concurrency (version column) for status transitions, `ON CONFLICT DO UPDATE` for window upsert. 44 unit tests (pure engine) + 53 PostgreSQL integration tests (real PostgreSQL, all passing). ATM: 15 of 15 scenarios PROVEN (advisory lock + partial unique index concurrency proof; worker runtime proof; operational stream integration proof; mechanical ATM integrity suite). Migration head `0032` (0031 = tables; 0032 = P0 active-incident uniqueness constraint). See [M19_ADVANCED_DDOS_DEFENSE_CENTER_REPORT.md](M19_ADVANCED_DDOS_DEFENSE_CENTER_REPORT.md), [M19_ADVERSARIAL_TRACEABILITY_MATRIX.md](M19_ADVERSARIAL_TRACEABILITY_MATRIX.md), [M19_COMPLETION_CHECKPOINT.md](M19_COMPLETION_CHECKPOINT.md). | 3,726 |
| M18 (COMPLETE) | Security Operations Command Center (evolution of M15) | See [M18_SECURITY_OPERATIONS_COMMAND_CENTER_REPORT.md](M18_SECURITY_OPERATIONS_COMMAND_CENTER_REPORT.md) and [M18_COMPLETION_CHECKPOINT.md](M18_COMPLETION_CHECKPOINT.md). Reconnaissance (9 parallel readers) established M15 already shipped a command center with a production-grade real-time backbone; M18 **evolves it and adds no second source of authoritative truth**. Reuses M15's SSE stream + query-time merged activity feed unchanged (no WebSockets/Kafka/Redis); surfaces the previously write-only M16 `network_drift_events` as the feed's **7th source** (tag "K") + a `GET /network-security/drift` endpoint. New `domain/command_center/`+`application/command_center/` bounded context holds only genuinely-new logic: a **deterministic, versioned, fully-explainable posture score** (`compute_posture_score`, formula v1.0.0 — weighted deduction over active-condition-by-severity + active-correlation counts, no ML/trend), plus closed enums. New read surfaces (all tenant-scoped, `organization_id` from token): `/command-center/overview` (posture + high-risk assets + inventory + zone counts), `/command-center/behavior` (deterministic UEBA over `organization_admin_audit_log` + HBA/NBA over `network_drift_events`, every signal linking to its exact source rows — no opaque scoring), `/command-center/integrations` (six provider-neutral external-telemetry boundaries — firewall, bandwidth/flow, ISP, backup/DR, threat-intel, geolocation — each **NOT_CONFIGURED until a real provider is wired; never fabricated telemetry**; register→AWAITING_TELEMETRY, config secret-stripped, audited), `/command-center/zones` (explicit admin-authored network-zone/DMZ classification — never IP-heuristic), and `/network-security/{top-ports,service-exposure}` (real aggregation over `network_observations`). Migration 0028: supporting observation index + `integration_providers` boundary table + `network_zone_assignments` (composite-FK tenant integrity). New premium dark command-center frontend (grouped permission-aware nav; `/command-center` overview with inline-SVG posture gauge + bars, behavior, exposure, integrations, DMZ/zones, and an inline-SVG network **relationship** map explicitly labelled *not an attack path*; honest loading/empty/not-configured/permission states; CSP-safe bundled SVG, no new frontend framework). **One real P1 concurrency defect found by adversarial testing and fixed**: `NetworkZoneRepository.upsert`/`IntegrationProviderRepository.upsert` used `merge()+flush()` with no `IntegrityError` handling, so concurrent same-key writes surfaced the unique-constraint loser as an unhandled HTTP 500 — fixed with the codebase's established SAVEPOINT + refetch-and-converge pattern (regression test: 8 concurrent zone assignments converge on one row, all handled). Adversarial suite: 27 integration tests (real PostgreSQL, real HTTP) + 15 posture unit cases — tenant isolation, RBAC (viewer vs owner), suspended-member denial, malformed-ID/pagination/filter bounds, posture determinism, top-ports/service-exposure aggregation correctness, integration secret-stripping/never-active, zone/DMZ, drift feed + live-feed merge, UEBA/HBA/NBA thresholds + evidence, concurrency. Clean migration `0001→0028→0027→0028` up/down/up proof. Real browser acceptance against live backend+frontend+PostgreSQL with seeded data: posture 74/moderate with live deduction breakdown, real KPIs/high-risk table/zone counts, top-ports + validated-services aggregation, DMZ overview, integrations honest NOT_CONFIGURED/AWAITING_TELEMETRY, inline-SVG map + gauge, grouped permission-aware nav, zero console errors. Disclosed limitation: the live SSE tick is subject to the same pre-existing Next-dev-proxy buffering as M15/M16 (feed data itself verified via poll endpoints + tests); host CPU/mem/disk and the six external integrations remain NOT_CONFIGURED (no source in-environment). Migration head `0028`. | 4,167 |
