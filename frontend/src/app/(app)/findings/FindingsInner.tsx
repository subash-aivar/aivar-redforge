...
export default function FindingsInner() {
  ...
  // Cross-module pivot target: `/findings?highlight=<finding_id>` — used by
  // the Risk Intelligence page's incident drawer (real `finding_ids` already
  // returned by `/api/v1/risk-incidents`) to jump straight to a specific
  // finding instead of leaving the operator to search for it manually.
  ...
  useEffect(() => {
    if (!highlightId || highlightedOnce.current || findings.length === 0) return;
    const target = findings.find((f) => f.id === highlightId);
    if (!target) return;
    highlightedOnce.current = true;
    void toggleFinding(target);
    rowRefs.current[highlightId]?.scrollIntoView?.({ block: "center" });
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [highlightId, findings]);
  ...
  return (
    <div>
      <PageHeader
        ...
        subtitle="Vulnerabilities discovered through AI red-team validation. Click a finding to inspect evidence."
      />
            ...
            return (
              <div
                ...
                className={`rounded-xl border bg-gray-900 transition-colors ${
                  f.id === highlightId ? "border-red-700 ring-1 ring-red-900/50" : "border-gray-800"
                }`}
              >
                <button
