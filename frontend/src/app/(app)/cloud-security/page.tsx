...
export default function CloudSecurityPage() {
  const [observations, setObservations] = useState<CloudSecurityObservation[] | null>(null);
  const [error, setError] = useState("");

  useEffect(() => {
    listCloudSecurityObservations()
      .then(setObservations)
      .catch(() => setError("UNAVAILABLE — failed to load cloud exposure observations."));
  }, []);
  ...
  return (
    <div>
      ...
      <p className="mt-1 text-sm text-gray-400">
        Deterministic observations over discovered cloud accounts/resources —
      ...
      {error ? (
        ...
        <div className="mt-6 rounded-xl border border-gray-800 bg-gray-900 p-8 text-center text-sm text-gray-500">
          No cloud exposure observations. Register an AWS connector and run discovery.
        </div>
      ) : (
